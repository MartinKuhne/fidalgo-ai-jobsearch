...
/// <summary>
/// Playwright-based browser automation tool for fetching web page content.
/// Handles Firefox browser management, sign-in detection for Indeed and Cloudflare, and HTML stripping.
/// Writes fetch logs to disk and returns sanitized content or error details.
/// </summary>
public class BrowserFetchTool : IBrowserFetchTool
{
    ...
    /// <summary>Initializes a new instance of the BrowserFetchTool.</summary>
    /// <param name="logger">Logger for diagnostic output.</param>
    /// <param name="htmlStripper">HTML stripper for content cleaning.</param>
        ...
        try
        {
            ...
            try
            {
               ...
               var initialUrl = request.Url;
                ...
                var detectedPageType = await DetectPageTypeAsync(page);

                if (detectedPageType == PageType.CloudflareChallenge)
                {
                    _logger.LogWarning("Cloudflare challenge detected at {Url}", page.Url);
                    var challengeContent = await page.ContentAsync();
                    var challengeStrippedContent = _htmlStripper.Strip(challengeContent);
                    var challengeTotalDuration = (int)(DateTime.UtcNow - startTime).TotalMilliseconds;

                    await WriteFetchLogAsync(fetchId, request.Url, challengeContent, "Cloudflare challenge detected", cancellationToken);

                    return new FetchResult(
                        Url: request.Url,
                        Content: challengeStrippedContent,
                        ContentLoadedAt: DateTime.UtcNow,
                        TotalDurationMilliseconds: challengeTotalDuration,
                        HasWaited: hasWaited,
                        WaitDurationMilliseconds: hasWaited ? waitDurationMs : null,
                        Error: "Cloudflare challenge detected. The page requires browser verification that could not be completed.",
                        PageType: PageType.CloudflareChallenge);
                }

                if (detectedPageType == PageType.SignInPage)
                {
                    var currentPageUrl = page.Url;
                    var pageTitle = await page.TitleAsync();
                    _logger.LogWarning("Sign-in page detected at {CurrentUrl} with title '{Title}'", currentPageUrl, pageTitle);
                }
   ...
   private async Task<bool> DetectSignInPageAsync(IPage page)
    {
        ...
        var isSignInTitle = titleLower.Contains("sign in");
    ...
    private async Task<PageType> DetectPageTypeAsync(IPage page)
    {
        ...
        var title = await page.TitleAsync();
        if (title.ToLowerInvariant().Contains("sign in"))
        {
    ...
    private async Task WriteFetchLogAsync(string fetchId, string url, string content, string? error, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning(ex, "Failed to write fetch log for {Url}", url);
        }
    }
}