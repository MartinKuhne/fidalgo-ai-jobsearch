...
/// <summary>
/// Scraper for glassdoor.com job postings.
/// </summary>
public class GlassdoorScraper : IWebsiteScraper
{
    ...
    public GlassdoorScraper(HttpClient httpClient)
    {
        _httpClient = httpClient;
        _context = BrowsingContext.New(AngleSharp.Configuration.Default);
        WebsiteName = "glassdoor.com";
    }
    ...
    {
        var searchUrl = $"https://www.glassdoor.com/Job/jobs.htm?suggestApp=true&src=JOB_SEARCH_INPUT_BOX.keywordInput&q={Uri.EscapeDataString(keyword)}";
    ...
    private List<JobPosting> ParseJobPostings(string html)
    {
        ...
        var jobElements = doc.QuerySelectorAll(".jobCardRoot, .jobListing, [data-testid='jobCardRoot']");

        foreach (var element in jobElements.Take(20))
        {
            var title = element.QuerySelector(".jobTitle, .jobTitleLink, a")?.TextContent?.Trim() ?? string.Empty;
            var company = element.QuerySelector(".employerName, .companyName")?.TextContent?.Trim() ?? string.Empty;
